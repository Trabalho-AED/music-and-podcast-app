...
from tkinter import ttk, filedialog # treeview
import time #Sleep
import random 
...
pathFormat = path_format() # Receber tipo de formato de pastas
...
isFavorite = False # Booleano que verifica se a musica a tocar está nos favoritos
...
def toggle_play():
    global isPaused  # Variável global para controlar o estado de pausa

    if isPaused:
        mixer.music.unpause()  # Retorna a música ao estado de reprodução
        btnPlay.configure(image=pauseIcon)  # Altera o ícone do botão para "pause"
        isPaused = False  # Atualiza o estado para não pausado
    else:
        mixer.music.pause()  # Pausa a música
        btnPlay.configure(image=playIcon)  # Altera o ícone do botão para "play"
        isPaused = True  # Atualiza o estado para pausado

def toggle_mute():
    global currentLevel  # Variável global para armazenar o nível de volume atual
    
    if volumeSlider.get() == 0:
        volumeSlider.set(currentLevel)  # Retorna ao volume anterior
        btnAudio.configure(image=audioIcon)  # Altera o ícone para "áudio"
    else:
        currentLevel = volumeSlider.get()  # Armazena o nível de volume atual
        volumeSlider.set(0)  # Silencia o áudio
        btnAudio.configure(image=muteIcon)  # Altera o ícone para "mudo"

    adjust_volume()  # Ajusta o volume com base no valor do slider
...
def login_action(usernameEntry, passwordEntry, resultLabel,loginFrame):
    ...
    if username == "not_found" and password == "not_found": # Se o utilizador e a password não existirem ou a password estiver errada
        resultLabel.configure(text="User does not exist or password is wrong.") # Texto a apresentar

    else: # Se a combinação utilizador password estiver correta 
        resultLabel.configure(text=f"Welcome {name}, Login successfull!\nUser Type: {adminflag}") # Texto a apresentar
...
def save_changes(usernameEntry, nameEntry, oldUsername, oldName):
    """Salva as alterações feitas nas credenciais do utilizador"""
    
    global usernameFinal, nameFull, btnUser, isAdmin
    
    # Atualiza as variáveis globais com as novas credenciais
    usernameFinal, nameFull = confirm_change(usernameEntry, nameEntry, usernameFinal, nameFull)
    
    oldPath = f"{usersPath}{oldUsername}{pathFormat}" 
    newPath = f"{usersPath}{usernameEntry.get()}{pathFormat}"

    os.rename(oldPath,newPath)

    # Atualiza o botão de utilizador com o novo nome
...
def create_playlist(playListName, playlistCreateFrame, errorLabel):
    ...
    # Caminho para o diretório onde as playlists são armazenadas
    playlistPath = f"{usersPath}{usernameFinal}{pathFormat}playlists{pathFormat}{playListName}.csv"

    # Verifica se o nome da playlist está vazio
    if playListName == " " or playListName == "":
        errorLabel.configure(text="O nome da playlist não pode ser vazio!")
        return
    
    # Verifica se a playlist já existe
    elif playListName + ".csv" in os.listdir(f"{usersPath}{usernameFinal}{pathFormat}playlists"):
        errorLabel.configure(text="A playlist já existe!")
        return
        ...
        # Destrói o frame de criação de playlist após a criação
...
def refresh_favorite(name, author, mainContentFrame, oldFrame):
    remove_favorite(name, author, usernameFinal)  # Remove a música dos favoritos
    favoritepage_render(mainContentFrame, oldFrame)  # Atualiza a página de favoritos

def refresh_playlist(name, author, mainContentFrame, oldFrame, playListName):
    remove_music_playlist(name, author, usernameFinal, playListName)  # Remove a música da playlist
    playlist_page_render(mainContentFrame, oldFrame, playListName)  # Atualiza a página da playlist
...
def register_render(oldFrame):
    ...
    nameLabel = customtkinter.CTkLabel(frameRegister, text="Name:", font=("Roboto", 18))
...
def mainwindow_render(oldFrame):
    ...
    global audioIcon,btnAudio,muteIcon,btnUser,playlistScrollFrame,favoriteIcon,noFavoriteIcon,likeBtn,mainContentFrame,currentFrame,nameFull,addIcon,playlistMenuFrame, musicName, artistName, musicLenSlider, volumeSlider, musicCover, playIcon, pauseIcon, btnPlay, playlistScrollFrame,playlistIcon # Variável global do frame em uso
    ...
    if isAdmin:
        #adfilestn = customtkinter.CTkButton(upperSearchFrame, width=100, height=10, fg_color="transparent", text="Add Music", command=add_music)
        #adfilestn.place(x=100, y=30)
    ...
    #Icon de Mute
    muteIcon = customtkinter.CTkImage(Image.open(f"{imagePath}mute_icon.png"), size=(20, 20))
    ...
    playListAdfilestn = customtkinter.CTkButton(playFrame,text="", width=35, height=35,image=addIcon ,fg_color="transparent", command=add_playlist_render)
    playListAdfilestn.place(x=95,y=50)
...
def search_frame(searchEntry, oldFrame):
    """Procura e apresenta músicas com base na procura"""
    
    global currentFrame  # Variável Global

    if oldFrame != None:
        oldFrame.destroy()  # Destruir frame antigo

    searchQuery = searchEntry.get().lower()  # Receber texto de pesquisa
    
    # Criar novo frame de pesquisa
    ...
    currentFrame = playlistPageFrame  # O frame atual a ser utilizado é o playlistPageFrame

    # Frame para exibir a música
    ...
    # Frame rolável para os itens de música
    ...
    musicLabel = customtkinter.CTkLabel(musicFrame, text=f"'{searchEntry.get()}' Resultados", font=("Roboto", 25))
    musicLabel.place(x=20, y=10)

    # Obter a lista de músicas e filtrá-la com base na consulta de pesquisa
    musicList = read_content("music")  # Obter dados de música

    # Filtrar os itens da playlist com base na consulta de pesquisa
    filteredPlaylist = []
    for music in musicList:
        musicName = music[0].lower()  # Converter para minúsculas para comparação sem distinguir maiúsculas/minúsculas
        musicAuthor = music[1].lower()
        musicCategory = music[2].lower()
        
        # Verificar se a consulta de pesquisa corresponde a algum campo (nome, autor ou categoria)
        if searchQuery in musicName or searchQuery in musicAuthor or searchQuery in musicCategory:
            filteredPlaylist.append(music)

    # Loop para criar "cartões" com base na pesquisa
    index = 0
    for music in filteredPlaylist:
        ...
        viewsLabel = customtkinter.CTkLabel(musicScrollFrame, text=f"{musicViews} Visualizações")
        viewsLabel.grid(row=index, column=4, padx=40, pady=20)

        index += 1  # Incrementar o índice para a próxima música
...
def userpage_render(mainContentFrame, oldFrame):
    ...
    # Frame para o Menu do Utilizador
    ...
    notificationsLabel = customtkinter.CTkLabel(notificationsFrame, text="Notificações:", font=("Arial", 30))
    ...
    set_check_var(usernameFinal, checkVarMusic, checkVarPodcast, checkVarOthers)  # Configurar os valores iniciais dos checkboxes

    # Checkboxes para opções de notificações
    checkboxMusic = customtkinter.CTkCheckBox(notificationsFrame, text="Músicas", variable=checkVarMusic, onvalue="on", offvalue="off")
    ...
    checkboxOthers = customtkinter.CTkCheckBox(notificationsFrame, text="Outros", variable=checkVarOthers, onvalue="on", offvalue="off")
    ...
    # Botão para guardar preferências
    savePreferencesBtn = customtkinter.CTkButton(notificationsFrame, text="Guardar Preferências", command=lambda: save_preferences(usernameFinal, checkVarMusic, checkVarPodcast, checkVarOthers))
    savePreferencesBtn.grid(row=0, column=2, rowspan=3)

def logout():
    """Função para deslogar o utilizador e limpar a interface"""
    
    # Destruir todos os widgets presentes na aplicação (limpar a interface)
    for widget in app.winfo_children():
        widget.destroy()
    
    # Chamar a função de renderização da página de login
    login_render(currentFrame)

def change_credentials(mainContentFrame, oldFrame):
    """Mostra a página para mudar as credenciais"""

    global currentFrame  # Variável global para o frame a ser usado

    oldFrame.destroy()  # Apagar o estilo do frame anterior

    # Frame para a página de mudança de credenciais
    ...
    # Botão para cancelar e voltar à página de utilizador
...
def manage_music_render(mainContentFrame, oldFrame):
    """Mostra a página de gestão de músicas"""
    ...
    currentFrame = musicManageFrame  # O frame a ser usado passa a ser o musicManageFrame

    # Label para mostrar o título "Manage Musics"
    ...
    # Atualizar a tabela com as músicas
    refresh_tree(tree, "music")

    # Para cada item na tree adicionar 1 ao contador
    counter=0
    for item in tree.get_children():
        counter+=1

    # Apresentar Label
    countLabel = customtkinter.CTkLabel(musicManageFrame, text=f"{counter} Musics.")
    countLabel.grid(row=4,column=0, pady=10)

def manage_podcast_render(mainContentFrame, oldFrame):
    """Mostra a página de gestão de podcasts"""
    ...
    currentFrame = podcastManageFrame  # O frame a ser usado passa a ser o podcastManageFrame

    # Label para mostrar o título "Manage Podcasts"
    trendingLabel = customtkinter.CTkLabel(podcastManageFrame, text="Manage Podcasts", font=("Roboto", 30))
    trendingLabel.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")

    # Botão para adicionar podcasts
    ...
    # Definir as colunas da tabela de podcasts
    ...
    # Atualizar a tabela com os podcasts
    refresh_tree(tree, "podcast")
    ...
    # Apresentar Label
    countLabel = customtkinter.CTkLabel(podcastManageFrame, text=f"{counter} Podcasts.")
    countLabel.grid(row=4,column=0, pady=10)

def manage_episodes_render(mainContentFrame, oldFrame):
    """Mostra a página de gerir episódios de podcasts"""
    ...
    # Frame de gestão de episódios de podcast
    podcastManageFrame = customtkinter.CTkScrollableFrame(mainContentFrame,
        orientation="vertical",
        width=1238,
        height=appHeight - (90 + 131),
    ...
    # Botão para excluir um episódio
    ...
    # Define as colunas da tabela de episódios
    ...
    # Organiza a árvore (tabela) de episódios
    tree.grid(row=1, rowspan=3, column=0, padx=70, pady=20, sticky='nsew')

    # Atualiza a tabela com os episódios
    refresh_tree(tree, "episodes")
    ...
    # Apresentar Label
    countLabel = customtkinter.CTkLabel(podcastManageFrame, text=f"{counter} Episodes.")
    countLabel.grid(row=4,column=0, pady=10)

def manage_users_render(mainContentFrame, oldFrame):
    """Mostra a página de gerir utilizadores"""
    ...
    currentFrame = usersManageFrame  # O frame a ser usado passa a ser o usersManageFrame

    # Label para mostrar o título "Manage Users"
    trendingLabel = customtkinter.CTkLabel(usersManageFrame, text="Manage users", font=("Roboto", 30))
    trendingLabel.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")

    # Botão para excluir utilizadores
    ...
    # Define as colunas da tabela de utilizadores
    ...
    # Organiza a árvore (tabela) de utilizadores
    tree.grid(row=1, rowspan=3, column=0, padx=70, pady=20, sticky='nsew')

    # Atualiza a tabela com os utilizadores
    ...
    # Apresentar Label
    countLabel = customtkinter.CTkLabel(usersManageFrame, text=f"{counter} Users.")
    countLabel.grid(row=4,column=0, pady=10)

def add_admin_render(tree):
    """Abre um frame para adicionar administradores"""

    # Frame para adicionar administrador
...
def manage_admins_render(mainContentFrame, oldFrame):
    """Mostra a página de gerir administradores"""
    ...
    # Criação do frame para a gestão de administradores
    usersManageFrame = customtkinter.CTkScrollableFrame(mainContentFrame,
        orientation="vertical",
        width=1238,
        height=appHeight - (90 + 131),
    ...
    # Botão para remover um administrador
    ...
    # Definir colunas para a tabela de administradores
    columns = ('username')
    ...
    # Organizar a árvore de administradores
    tree.grid(row=1, rowspan=3, column=0, padx=70, pady=20, sticky='nsew')

    # Textbox para exibir e editar notificações
    ...
    # Botões para adicionar e deletar notificações
    ...
    # Atualiza a tabela com os administradores
    ...
    # Apresentar Label
    countLabel = customtkinter.CTkLabel(usersManageFrame, text=f"{counter} Admins.")
    countLabel.grid(row=2,column=1, pady=10)

def add_playlist_render():
    # Cria um frame onde as playlists serão mostradas
    playlistFrame = customtkinter.CTkFrame(app, height=170, width=170, fg_color="#0A090C")
    playlistFrame.place(x=150, y=500)

    # Cria um frame rolável dentro do frame das playlists
    playlistScrollFrame = customtkinter.CTkScrollableFrame(
        playlistFrame,
        orientation="vertical",
        width=150,
    ...
    # Obtém a lista de playlists do usuário
    playLists = get_playlists()
    
    # Cria um botão para cada playlist
    for i in range(len(playLists)):
        # Cria um botão para cada playlist
        btnPlaylist1 = customtkinter.CTkButton(
            playlistScrollFrame,
            image=playlistIcon,  # Ícone da playlist
            width=31, height=31,
            fg_color="transparent",
            text=f"{playLists[i]}",  # Nome da playlist
...
def add_to_playlist(playlistName, playlistFrame):
    # Lê o arquivo da playlist selecionada
    ...
    for line in lines:
        ...
        if fields[0] == musicNameCurrent and fields[1] == musicAuthorCurrent:
            return  # A música já está na playlist, nada é feito

    # Se a música não está na playlist, ela é adicionada
    ...
    # Fecha o frame de playlists após adicionar
    playlistFrame.destroy()

def manage_categories_render(mainContentFrame, oldFrame):
    """Mostra a página de gerir categorias"""

    global currentFrame  # Variável global para frame a ser usado

    if oldFrame != None:
        oldFrame.destroy()  # Apaga o estilo do frame anterior
    ...
    currentFrame = categoriesManageFrame  # O frame a ser usado passa a ser o categoriesManageFrame

    # Label para mostrar o texto "Manage Categories"
    ...
    # Botão para excluir uma categoria
    ...
    # Define as colunas da árvore (treeview)
    ...
    # Chama a função para atualizar a árvore de categorias
    ...
    # Apresentar Label
    countLabel = customtkinter.CTkLabel(categoriesManageFrame, text=f"{counter} Categories.")
    countLabel.grid(row=4,column=0, pady=10)

def add_categories_render(tree):
    """Abre um frame para adicionar categorias"""
    ...
    # Label para mostrar o texto "Category Name:"
    ...
    # Entry para o nome da categoria
...
def adminpage_render(mainContentFrame, oldFrame):
    """Mostra a homepage de administração"""
    ...
    currentFrame = homepageFrame  # O frame a ser usado passa a ser o homepageFrame
    
    # Configura as linhas e colunas para centralizar os elementos
    ...
    # Label para o título "Admin Dashboard"
...
def homepage_render(mainContentFrame, oldFrame):
    ...
    # Criação de um scrollable frame dentro do frame principal
    ...
    # Criar botões para cada música em alta
    musicList = read_content("music")  # Receber dados da lista de músicas
    ...
    # Limitar a exibição às 10 músicas com mais visualizações
    topTrendingMusic = musicList[:10]

    # Loop para criar os botões para as músicas
    index = 0
    ...
    # Abrir lista de podcasts disponíveis
    podcastList = read_content("podcast") # Receber dados da lista de podcasts

    # Converter as visualizações para inteiro e ordenar manualmente
    for podcast in podcastList:
        podcast[2] = int(podcast[2])  # Converter o campo de visualizações (índice 2) para inteiro

    # Ordenar a lista de podcasts pelo número de visualizações, em ordem decrescente
    ...
    # Limitar a exibição aos 5 podcasts com mais visualizações
    topTrendingpodcast = podcastList[:5]

    # Loop para criar os botões para os podcasts
    index = 0
    for podcast in topTrendingpodcast:
        ...
        # Limitar o nome do episódio e podcast se for demasiado longo
    ...
    # Título da secção "Discover"
    MusicLabel = customtkinter.CTkLabel(MusicDiscoverFrame, text="Discover", font=("Roboto", 25))
    MusicLabel.place(x=10, y=0)

    # Limitar a exibição a 8 itens (podcasts ou músicas)
    randomItems = []

    # Escolher aleatoriamente entre podcasts e músicas
    for _ in range(4):  # Limitar a 4 itens de cada tipo
        randomMusic = random.choice(musicList)
        randomPodcast = random.choice(podcastList)

        randomItems.append(('music', randomMusic))
        randomItems.append(('podcast', randomPodcast))

    # Embaralhar a lista para aleatoriedade
    random.shuffle(randomItems)

    # Loop para criar os botões de músicas e podcasts aleatórios
    index = 0
    for itemType, item in randomItems:
        if itemType == 'music':
            musicName = item[0]
            musicAuthor = item[1]
            musicCategory = item[2]
            musicViews = item[3]
            musicCover = coverArtPath + item[4]
            musicURL = item[5]
            ...
            button = customtkinter.CTkButton(
                ...
                fg_color="transparent",
                compound="top",
                command=lambda idx=index, playlist=randomItems: play_music(idx, playlist)
            )

        elif itemType == 'podcast':
            podcastEpisode = item[0]
            podcastName = item[1]
            podcastViews = item[2]
            podcastCover = coverArtPath + get_cover_art(podcastName)
            podcastURL = item[3]
...
def musicpage_render(mainContentFrame, oldFrame):
    """Mostra a homepage de música"""

    global currentFrame, currentPlaylist  # Variáveis globais para o frame atual e a playlist

    if oldFrame != None:
        oldFrame.destroy()  # Apagar o frame antigo, caso exista
    
    # Criar o frame principal da página de música
    MusicpageFrame = customtkinter.CTkScrollableFrame(mainContentFrame,
        orientation="vertical",  # Orientação do scroll será vertical
        width=1238,  # Largura do frame
        height=appHeight - (90 + 131),  # Altura ajustada com base na altura da aplicação
        fg_color="black",  # Cor de fundo preta
        corner_radius=0  # Sem cantos arredondados
    )
    MusicpageFrame.place(x=0, y=0)  # Posicionar o frame no topo à esquerda

    currentFrame = MusicpageFrame  # Atualizar a variável global com o frame atual

    # Frame para o menu "Músicas"
    MusicFrame = customtkinter.CTkFrame(MusicpageFrame, width=1300, height=300, fg_color="transparent", corner_radius=0)
    MusicFrame.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")  # Posicionar com margem

    # Criar um frame rolável dentro do frame principal
    MusicScrollFrame = customtkinter.CTkScrollableFrame(
        MusicFrame,
        orientation="horizontal",  # O scroll será horizontal
        width=1200,  # Largura do frame rolável
        height=250,  # Altura do frame rolável
        fg_color="transparent"  # Sem cor de fundo
    )
    MusicScrollFrame.place(x=0, y=0)  # Posicionar o frame rolável

    MusicLabel = customtkinter.CTkLabel(MusicFrame, text="Music", font=("Roboto", 25))  # Rótulo "Música"
    MusicLabel.place(x=20, y=10)  # Posicionar o rótulo

    # Obter a lista de músicas
    musicList = read_content("music")  # Função que retorna uma lista de músicas

    index = 0
    for music in musicList:
        # Desestruturar cada música da lista
        musicName = music[0]
        musicAuthor = music[1]
        musicCategory = music[2]
        musicViews = music[3]
        ...
        # Criar a imagem da capa do álbum
        coverArt = customtkinter.CTkImage(Image.open(musicCover), size=(150, 150))

        # Criar um botão para cada música
        button = customtkinter.CTkButton(
            MusicScrollFrame,
            width=150,  # Largura do botão
            height=150,  # Altura do botão
            text=f"{musicName}\n{musicAuthor}",  # Texto do botão com nome e autor
            image=coverArt,  # Imagem da capa
            fg_color="transparent",  # Cor de fundo transparente
            compound="top",  # Imagem em cima do texto
            command=lambda idx=index: play_music(idx, musicList)  # Função para tocar a música ao clicar
        )

        button.grid(row=0, column=index, padx=10, pady=40)  # Posicionar o botão na grid
        index += 1  # Incrementar o índice para o próximo botão

    # Frame para "Your Activity" (Atividade recente do usuário)
    ...
    # Criar um frame rolável dentro da secção de atividade
    MusicScrollFrame = customtkinter.CTkScrollableFrame(
        MusicYourActivityFrame,
    ...
    # Rótulo para "Your Activity"
    MusicLabel = customtkinter.CTkLabel(MusicYourActivityFrame, text="Your Activity", font=("Roboto", 25))
    MusicLabel.place(x=10, y=0)

    # Caminho para o arquivo de atividades do usuário
    activityPath = f"{usersPath}{usernameFinal}{pathFormat}music_activity.csv"

    # Obter as músicas recentes do usuário
    recentMusic = get_recent_songs(activityPath)
    activityList = filter_music(musicList, recentMusic)  # Filtrar músicas da lista com base na atividade
    ...
    for music in activityList:
        ...
        button = customtkinter.CTkButton(
            ...
            fg_color="transparent",
            compound="top",
            command=lambda idx=index: play_music(idx, activityList)  # Passar o índice para a função de tocar
        )

        button.grid(row=0, column=index, padx=10, pady=40)
        index += 1  # Incrementar o índice

    # Frame para "Discover" (Descobrir músicas)
    ...
    # Criar um frame rolável para descobrir músicas
    MusicScrollFrame = customtkinter.CTkScrollableFrame(
        MusicDiscoverFrame,
        orientation="horizontal",  # Scroll horizontal
        width=1200,
    ...
    # Rótulo para "Discover"
    MusicLabel = customtkinter.CTkLabel(MusicDiscoverFrame, text="Discover", font=("Roboto", 25))
    MusicLabel.place(x=10, y=0)

    # Seleciona aleatoriamente 4 músicas para a secção "Discover"
    ...
    for music in randomMusic:
        ...
        button = customtkinter.CTkButton(
            ...
            fg_color="transparent",
            compound="top",
            command=lambda idx=index: play_music(idx, randomMusic)  # Função para tocar música aleatória
        )
    ...
    # Loop principal para mostrar músicas por categoria
    categoryList = get_categories_music(musicList)  # Obter todas as categorias únicas
    index = 2  # Começar após as secções "Music", "Your Activity" e "Discover"

    for category in categoryList:
        print(f"Processing category: {category}")  # Mensagem de debug

        # Criar o frame para cada categoria
        ...
        # Criar um scrollable frame para as músicas dessa categoria
        ...
        MusicLabel = customtkinter.CTkLabel(musicCategoryFrame, text=f"{category}", font=("Roboto", 25))  # Rótulo da categoria
        MusicLabel.place(x=10, y=0)

        # Filtrar as músicas dessa categoria
        categoryMusicList = [music for music in musicList if music[2] == category]

        print(f"Songs in category '{category}': {len(categoryMusicList)}")  # Mensagem de debug

        if categoryMusicList:
            col_index = 0
            for music in categoryMusicList:
                ...
                button = customtkinter.CTkButton(
                    ...
                    fg_color="transparent",
                    compound="top",
                    command=lambda idx=col_index, playlist=categoryMusicList: play_music(idx, playlist)  # Passar lista de músicas da categoria
                )

                button.grid(row=0, column=col_index, padx=10, pady=40)
                col_index += 1  # Incrementar índice

        else:
            MusicLabel = customtkinter.CTkLabel(musicCategoryFrame, text="No songs available for this category.", font=("Roboto", 20))  # Mensagem caso não haja músicas
            MusicLabel.place(x=10, y=0)

        index += 1  # Incrementar o índice para a próxima categoria
        lastIndex = index
    
    # Obter lista de autores únicos
    authorList = get_authors(musicList)

    index = lastIndex + 1  # Começar após as secções anteriores
    for author in authorList:
        ...
        MusicLabel = customtkinter.CTkLabel(musicAuthorFrame, text=f"{author}", font=("Roboto", 25))  # Rótulo com o nome do autor
        MusicLabel.place(x=10, y=0)

        # Filtrar as músicas do autor
        ...
        for music in authorMusicList:
            ...
            button = customtkinter.CTkButton(
                ...
                fg_color="transparent",
                compound="top",
                command=lambda idx=col_index, playlist=authorMusicList: play_music(idx, playlist)  # Passar lista de músicas do autor
            )
...
def delete_playlist(playListName, playlistPageFrame):
    """Apaga a playlist especificada"""

    # Apaga o arquivo da playlist do disco
    os.remove(f"{usersPath}{usernameFinal}{pathFormat}playlists{pathFormat}{playListName}.csv")
    print(f"Removed {usersPath}{usernameFinal}{pathFormat}playlists{pathFormat}{playListName}.csv.")  # Mensagem de confirmação

    # Atualiza a página inicial, carregando novamente o conteúdo
    homepage_render(mainContentFrame, currentFrame)

    # Atualiza a lista de playlists na página de playlists
    refresh_playlists(playlistScrollFrame)

def playlist_page_render(mainContentFrame, oldFrame, playListName):
    """Mostra a página da playlist especificada"""

    global currentFrame  # Variável global para armazenar o frame a ser utilizado

    if oldFrame != None:
        oldFrame.destroy()  # Apaga o frame anterior, removendo-o da interface
    
    # Criação do frame para a página da playlist
    playlistPageFrame = customtkinter.CTkFrame(
        mainContentFrame,
        width=1238,
        height=appHeight - (90 + 131),
    ...
    currentFrame = playlistPageFrame  # O frame atual será o frame da playlist

    # Frame principal para a lista de músicas
    MusicFrame = customtkinter.CTkFrame(playlistPageFrame, width=1300, height=800, fg_color="transparent", corner_radius=0)
    MusicFrame.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")

    # Criação de um frame rolável dentro do frame principal
    MusicScrollFrame = customtkinter.CTkScrollableFrame(
        MusicFrame,
        orientation="vertical",
        width=1200,
    ...
    # Botão para apagar a playlist
    playlistDeleteBtn = customtkinter.CTkButton(
        MusicFrame,
        text="Delete Playlist",
        font=("Roboto", 25),
        command=lambda: delete_playlist(playListName, playlistPageFrame)  # Apaga a playlist ao ser clicado
    )
    playlistDeleteBtn.place(x=200, y=10)

    # Cria uma lista de músicas associadas à playlist
    musicList = read_content("music")  # Recebe a lista de músicas disponíveis
    playlistList = get_playlist_list(musicList, usernameFinal, playListName)  # Filtra as músicas da playlist

    # Loop para criar os botões para cada música da playlist
    index = 0
    for music in playlistList:
        ...
        # Adiciona a capa da música ao botão
        coverArt = customtkinter.CTkImage(Image.open(musicCover), size=(150, 150))

        # Criação do botão para tocar a música
        button = customtkinter.CTkButton(
            MusicScrollFrame,
            width=150,
            height=150,
            text="",  # Texto vazio pois a imagem já é o principal elemento
            image=coverArt,
            fg_color="transparent",
            compound="top",
            command=lambda idx=index, playlist=playlistList: play_music(idx, playlist)  # Passa o índice e a playlist para a função
        )
        button.grid(row=index, column=0, padx=40, pady=20)

        # Exibe o nome da música ao lado da capa
        ...
        # Exibe o nome do autor ao lado da música
        ...
        # Exibe o número de visualizações da música
        ...
        # Botão para remover a música da playlist
        button2 = customtkinter.CTkButton(
            MusicScrollFrame,
            width=150,
            height=50,
            text="Remove from Playlist",  # Texto do botão
            command=lambda name=musicName, author=musicAuthor: refresh_playlist(name, author, mainContentFrame, currentFrame, playListName)  # Remove a música ao clicar
        )
        button2.grid(row=index, column=4, padx=40, pady=20)

        index += 1  # Incrementa o índice para o próximo botão/música

def favoritepage_render(mainContentFrame, oldFrame):
    """Mostra a página dos favoritos"""
    ...
    # Criação do frame para a página de favoritos
    favoritePageFrame = customtkinter.CTkFrame(
        mainContentFrame,
        width=1238,
        height=appHeight - (90 + 131),
        fg_color="black",
    ...
    currentFrame = favoritePageFrame  # O frame atual será o frame de favoritos
    ...
    # Adiciona o título da lista de favoritos
    MusicLabel = customtkinter.CTkLabel(MusicFrame, text=f"{nameFull} Favorite List", font=("Roboto", 25))
    MusicLabel.place(x=20, y=10)

    # Cria uma lista de músicas favoritas
    musicList = read_content("music")  # Recebe a lista de músicas disponíveis
    favoritesList = get_favorites(musicList, usernameFinal)  # Filtra as músicas favoritas

    # Loop para criar os botões para cada música favorita
    index = 0
    for music in favoritesList:
        ...
        button = customtkinter.CTkButton(
            ...
            fg_color="transparent",
            compound="top",
            command=lambda idx=index, playlist=favoritesList: play_music(idx, playlist)  # Passa o índice e a lista para a função
        )
        ...
        # Exibe a categoria da música ao lado do autor
        categoryLabel = customtkinter.CTkLabel(
            MusicScrollFrame,
            text=f"{musicCategory}"
        )
        categoryLabel.grid(row=index, column=3, padx=40, pady=20)
        ...
        button2 = customtkinter.CTkButton(
            ...
            width=150,
            height=50,
            text="Remove from Favorites",  # Texto do botão
            command=lambda name=musicName, author=musicAuthor: refresh_favorite(name, author, mainContentFrame, oldFrame)  # Remove a música ao clicar
        )
        button2.grid(row=index, column=5, padx=40, pady=20)
